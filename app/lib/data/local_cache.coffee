# checking window.localdb_ for dependency injecting in unit tests
localdb_ = window.localdb_ or require 'lib/data/local_db'
# constructor for interactions between module and LevelDb/IndexedDb
LocalDB = localdb_.build window, _
...
module.exports = (options)->
  ...
  API =
    get: (ids, format, refresh)->
      ...
      .catch _.ErrorRethrow("#{name} local.get err")
  ...
  findMissingIds = (data)->
    ...
    missingIds = []
    for k, v of data
      unless v? then missingIds.push k
  ...
  if remote.post?
    API.post = (data)->
      ...
      .then (res)->
        # RES IS UNDEFINED
        # => status code >= 400 should return a REJECTED PROMISE
        _.log res, "cache:#{name}:post res"
        id = findId res
        putInLocalDb id, res
      .catch _.ErrorRethrow("#{name} local.post err")
