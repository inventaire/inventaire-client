<script lang="ts">
  ...
  const initialMapViewPosition = [ 45.744, 4.821 ]
  ...
  async function fetchItemsUsersAndGroups () {
    ...
    ({ users, items } = await app.request('items:getByArea', { items, bbox, lang })
      .catch(err => {
        if (err.message !== 'no items found') flash = err
      }))
    groups = await getDocsByPosition('groups', bbox)
  }
