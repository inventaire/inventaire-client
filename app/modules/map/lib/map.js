...
export default map_ = {
  ...
  showOnMap(typeName, map, models){
    if (typeName === 'users') { return map_.showUsersOnMap(map, models);
    } else if (typeName === 'groups') { return map_.showGroupsOnMap(map, models);
  ...
  showModelsOnMap(map, models){
    return (() => {
      const result = [];
      for (let model of _.forceArray(models)) {
  ...
  showUsersOnMap(map, users){
    return _.forceArray(users).map((user) =>
      map_.showUserOnMap(map, user));
  },

  showGroupsOnMap(map, groups){
    return _.forceArray(groups).map((group) =>
