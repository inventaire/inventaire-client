...
const requestNewTasks = async (type, entitiesType, limit, offset) => {
  const tasksRes = await preq.get(app.API.tasks.byEntitiesType({
    type,
    'entities-type': entitiesType,
    limit,
    offset
  }))
  // return autogenerated tasks if no user/reporter ones are available
  // as `byScore` endpoint should only return those
  if (tasksRes.tasks.length === 0 && entitiesType === 'human') {
