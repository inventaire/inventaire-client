...
var API = {
  showHome() {
    if (app.user.loggedIn) { return app.execute('show:inventory:main:user');
...
var requireAdminAccess = function() {
  setPrerenderStatusCode(401);
  if (app.user.hasAdminAccess) { return true;
  } else {
    showErrorNotAdmin();
    return false;
  }
...
var requireDataadminAccess = function() {
  setPrerenderStatusCode(401);
  if (app.user.hasDataadminAccess) { return true;
  } else {
    showErrorNotAdmin();
    return false;
  }
...
var showErrorByStatus = function(err, label){
  if (err.statusCode === 404) { return showErrorMissing();
...
var showErrorCookieRequired = command => showError({
  ...
  redirection: {
    ...
    classes: 'dark-grey',
    buttonAction() {
      if (command != null) { return app.execute(command);
...
var showCallToConnection = message => app.layout.modal.show(new CallToConnection({
