...
export default function(app){
  ...
  const async = {
    getUserModel(id, refresh){
      ...
      if ((model != null) && !refresh) { return Promise.resolve(model);
      } else {
    ...
    getUsersModels(ids){
      ...
      for (let id of ids) {
        ...
        if (userModel != null) { foundUsersModels.push(userModel);
    ...
    resolveToUserModel(user){
      ...
      let promise;
      if (_.isModel(user)) {
        if (user.get('username') != null) { return Promise.resolve(user);
