...
export default UserCommons.extend({
  ...
  calculateHighlightScore() {
    ...
    const freshnessFactor = 100 / (_.daysAgo(itemsLastAdded) + 1);
...
var getStatus = function(id, relations){
  if (relations.friends.includes(id)) { return 'friends';
  } else if (relations.userRequested.includes(id)) { return 'userRequested';
  } else if (relations.otherRequested.includes(id)) { return 'otherRequested';
  } else if (relations.network.includes(id)) { return 'nonRelationGroupUser';
  } else { return 'public'; }
};
