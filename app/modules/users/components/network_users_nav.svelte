  ...
  <div class="list-wrapper">
    <div class="list-label-wrapper">
      ...
      <div id="userMenu" class="buttons">
        {#if showUsersMenu}
          <button
            on:click={app.Execute('show:invite:friend:by:email')}
          >
            {@html icon('envelope')}
            {I18n('invite')}
          </button>
          <button
            on:click={app.Execute('show:users:search')}
          >
            {@html icon('search')}
            {I18n('search')}
          </button>
          <button
            on:click={app.Execute('show:users:nearby')}
          >
  ...
  <div class="list-wrapper">
    <div class="list-label-wrapper">
      ...
      <div class="buttons groupMenu">
        {#if showGroupsMenu}
          <button
            on:click={app.Execute('show:groups:search')}
          >
            {@html icon('search')}
            {I18n('search')}
          </button>
          <button
            on:click={app.Execute('show:groups:nearby')}
          >
            {@html icon('map-marker')}
            {I18n('find on map')}
          </button>
