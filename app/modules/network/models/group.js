...
export default Positionable.extend({
  ...
  requestsCount() {
    ...
    if (this.mainUserIsAdmin()) { return this.get('requested').length;
    } else { return 0; }
  },
  ...
  userStatus(user){
    ...
    const { id } = user;
    if ((needle = id, this.allMembersIds().includes(needle))) { return 'member';
    } else if ((needle1 = id, this.allInvitedIds().includes(needle1))) { return 'invited';
    } else if ((needle2 = id, this.allRequestedIds().includes(needle2))) { return 'requested';
    } else { return 'none'; }
  },
  ...
  getDescription() {
    ...
    if (_.isNonEmptyString(desc)) { return desc;
