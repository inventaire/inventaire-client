...
fetchAllItems = (model)->
  ...
  if model.hasSubentities
    ...
    # Then 'items:byEntity' will take care of making every request only once.
    ...
    .then -> app.request 'items:byEntity', model.get('allUris')
    .catch _.Error("#{mainUri} subentities fetchPublicItems err")

  else
    return app.request 'items:byEntity', [ mainUri ]
