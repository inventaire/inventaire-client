...
customFieldMatch = (inputBasedRegex, rawInput)-> (field)->
  unless field? then return false
  if not rawInput? and _.isString inputBasedRegex then rawInput = inputBasedRegex
  entityId = getEntityId rawInput
  fieldContainsInput = field.match(inputBasedRegex)?
