...
export default {
  ...
  normalizeUri(uri){
    ...
    if ((id == null)) {
      if (wdk.isWikidataItemId(prefix)) { [ prefix, id ] = Array.from([ 'wd', prefix ]);
      } else if (_.isInvEntityId(prefix)) { [ prefix, id ] = Array.from([ 'inv', prefix ]);
    ...
    if ((prefix != null) && (id != null)) { return `${prefix}:${id}`;
    } else { return uri; }
  }
};
