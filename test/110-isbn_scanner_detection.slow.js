...
describe('isbn scanner detection', function() {
  it('should not add an isbn on the first scan', function(done){
    const addedIsbns = [];
  ...
  it('should add an isbn after 2 successive scans', function(done){
    const addedIsbns = [];
    const addIsbn = result => addedIsbns.push(result);
    const onDetected = OnDetected({ addIsbn });
    onDetected(fakeResult(validIsbn));
  ...
  it('should not add an isbn if preceded by a different valid isbn', function(done){
    const addedIsbns = [];
    const addIsbn = result => addedIsbns.push(result);
    const onDetected = OnDetected({ addIsbn });
    onDetected(fakeResult(validIsbn));
  ...
  return it('should add an isbn after 2 successive scans, even if another valid isbn was scanned before', function(done){
    const addedIsbns = [];
    const addIsbn = result => addedIsbns.push(result);
    const onDetected = OnDetected({ addIsbn });
    onDetected(fakeResult(validIsbn));
